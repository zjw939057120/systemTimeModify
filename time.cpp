...
std::string sysTime1;							// 时间
std::string sysTime2;							// 时间
...
int			GetNetTime();//获取网络时间
BOOL			SetSysTime();//设置系统时间
                     ...
                     _In_ int       nCmdShow)
{
	...
	GetNetTime();
	if (SetSysTime())
		MessageBox(nullptr, _T("系统时间设置成功！"), _T("提示信息"), MB_OK|MB_ICONINFORMATION);
	else
		MessageBox(nullptr, _T("系统时间设置失败！"), _T("提示信息"), MB_OK|MB_ICONERROR);
	PostQuitMessage(0);
	return 0;
...
int GetNetTime(){
	...
	if (!res || res->status != 200) {
		MessageBox(nullptr, _T("网络请求失败！"), _T("提示信息"), MB_OK|MB_ICONERROR);
		PostQuitMessage(0);
	}
		...
		if (!reader->parse(rawJson.c_str(), rawJson.c_str() + rawJsonLength, &root,
			&err)) {
			MessageBox(nullptr, _T("时间获取失败！"), _T("提示信息"), MB_OK|MB_ICONERROR);
			PostQuitMessage(0);
